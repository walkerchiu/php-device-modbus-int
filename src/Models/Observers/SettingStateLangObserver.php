...
namespace WalkerChiu\DeviceModbus\Models\Observers;

class SettingStateLangObserver
{
    /**
    ...
    {
        $query =
            config('wk-core.class.device-modbus.settingStateLang')
                ::withTrashed()
        ...
        if (
            config('wk-device-modbus.soft_delete')
            && (
                config('wk-core.lang_log')
                || config('wk-device-modbus.lang_log')
            )
        ) {
